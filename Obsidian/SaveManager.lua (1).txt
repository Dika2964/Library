...
local SaveManager = {} do
    ...
    function SaveManager:ImportConfig(configString)
        ...
        local success, configData = pcall(httpService.JSONDecode, httpService, configString)
        if not success or not configData.objects then
            return false, "invalid config format"
        end

        local applied = 0
        for _, option in pairs(configData.objects) do
            if option.type and self.Parser[option.type] then
                local success = pcall(self.Parser[option.type].Load, option.idx, option)
                if success then applied = applied + 1 end
            end
        end

        return true, "imported " .. applied .. " settings"
    end
    ...
    function SaveManager:BuildConfigSection(tab)
        ...
        section:AddButton("Export config", function()
            local success, configString = self:ExportConfig()
            if not success then
                return self.Library:Notify("Failed to export config: " .. configString)
            end

            -- Copy to clipboard if available
            if setclipboard then
                setclipboard(configString)
                self.Library:Notify("Config exported and copied to clipboard!")
            else
                self.Library:Notify("Config exported (clipboard not available)")
            end

            -- Also show the config string in the input field
            if self.Library.Options.SaveManager_ImportBox then
                self.Library.Options.SaveManager_ImportBox:SetValue(configString)
            end
        end)
        ...
        section:AddButton("Import config", function()
            local configString = self.Library.Options.SaveManager_ImportBox.Value

            if not configString or configString:gsub(" ", "") == "" then
                return self.Library:Notify("Please paste a config string first", 2)
            end

            local success, result = self:ImportConfig(configString)
            if not success then
                return self.Library:Notify("Failed to import config: " .. result)
            end

            self.Library:Notify("Successfully " .. result)
            -- Clear the import box
            self.Library.Options.SaveManager_ImportBox:SetValue("")
        end)
