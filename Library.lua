...
local Templates = {
    ...
    Window = {
        ...
        ShowCustomCursor = true,
        Gradient = {
             Enabled = true,       -- aktifkan gradasi global (true/false)
             Brightness = 0.8,     -- seberapa terang ujung kanan jadi lebih terang (0..1)
             Rotation = 90,         -- 0 = horizontal (kiri->kanan)
        },
        Font = Enum.Font.Code,
        ToggleKeybind = Enum.KeyCode.RightControl,
...
-- AddGradient helper: menambah UIGradient horizontal dari BaseColor -> versi lebih terang
local function AddGradient(Object, BaseColor, SettingsTable)
    if not Object or not BaseColor then return end

    -- cari settings: prioritas runtime SettingsTable -> Library.Template.Window -> global Templates.Window
    local Settings = nil
    Settings = (SettingsTable and SettingsTable.Gradient) and SettingsTable.Gradient or (Library and Library.Gradient) or (Templates and Templates.Window and Templates.Window.Gradient)
    ...
    gradient.Rotation = Settings.Rotation or 0

    local brightness = tonumber(Settings.Brightness) or 0.2
    local function brighten(c)
        return Color3.new(
            math.clamp(c.R + brightness, 0, 1),
            math.clamp(c.G + brightness, 0, 1),
            math.clamp(c.B + brightness, 0, 1)
        )
    end

    local brighter = brighten(BaseColor)
    gradient.Color = ColorSequence.new(BaseColor, brighter)
...
do
    ...
    function Funcs:AddCheckbox(Idx, Info)
        ...
        AddGradient(Checkbox, Library.Scheme.MainColor, Templates)
        New("UICorner", {
            CornerRadius = UDim.new(0, Library.CornerRadius / 2),
            Parent = Checkbox,
        })
    ...
    function Funcs:AddToggle(Idx, Info)
        ...
        AddGradient(Switch, Library.Scheme.MainColor, Templates)
        New("UICorner", {
            CornerRadius = UDim.new(1, 0),
            Parent = Switch,
        })
    ...
    function Funcs:AddInput(Idx, Info)
        ...
        AddGradient(Box, Library.Scheme.MainColor, Templates)
    ...
    function Funcs:AddSlider(Idx, Info)
        ...
        AddGradient(Fill, Library.Scheme.MainColor, Templates)
